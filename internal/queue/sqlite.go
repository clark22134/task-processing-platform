...
func (r *sqliteRepo) Enqueue(ctx context.Context, t domain.Task) (string, error) {
	id := t.ID
	if id == "" { id = "tsk_" + uuid.NewString() }
	if t.Priority == 0 { t.Priority = 5 }
	if t.MaxAttempts == 0 { t.MaxAttempts = 5 }
	if t.VisibilityTimeout == 0 { t.VisibilityTimeout = 60 }
...
func (r *sqliteRepo) LeaseNext(ctx context.Context, now time.Time) (domain.Task, Lease, error) {
	...
	if err != nil { return domain.Task{}, Lease{}, err }
...
`, now)
	...
	if err == sql.ErrNoRows { return domain.Task{}, Lease{}, tx.Rollback(); }
	if err != nil { return domain.Task{}, Lease{}, err }
	if idem.Valid { s := idem.String; t.IdempotencyKey = &s }
	...
	if err = tx.Commit(); err != nil { return domain.Task{}, Lease{}, err }
